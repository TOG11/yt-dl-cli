...
const open = require('open');
  if (warn)
  console.log(colors.redBright("[TYTD@BETA] WARNING: YOU ARE ON A BETA BRANCH, THIS BRANCH IS STILL BEING UPDATED DAILY!\n\nPLEASE MAKE SURE TO UPDATE THE BRANCH DAILY BY USING 'yt --branch beta_branch' !!!!!!"))
  warn = false;
...
function RUN() {
  ...
  fs.writeFile("./package.json", updatedBin, function writeJSON(err) {
      if (err) return console.log(err);
      childProcess.exec("cd "+__dirname+' & npm i . -g')
      console.log(colors.green("\n\n[TYTD] Added Start Command "+colors.magentaBright(options.addcommand)));
    });
} else if (options.about) {
  request('http://togi-cloud.ngrok.io/TYTD/PATHOS3/info', { json: true }, (err, body) => {
    if (err) { return console.log(err); }
...
video.on('close', function () {
  ...
  if (isAudioDone & isVideoDone) {
    ...
    var process = childProcess.exec(`${__dirname}/ffmpeg.exe & ffmpeg -i ${Documents}/TYTD/MP4/${fileIdentifier}_MP4/1.mp4 -i ${Documents}/TYTD/MP3/${fileIdentifier}_MP3/1.mp3 -b:a ${audioBitrate}k -ar ${audioHz} -c:v libx264 -c:a aac -preset ultrafast ${home}\\Videos\\TYTD\\${fileIdentifier}_yt-dl-cli.mp4`)
...
require('child_process').exec(`start "" "${home}\\Videos\\TYTD\\"`);
    ...
    multibar.remove(b1);
        ...
        //if audio is done after video, combine both. else, wait for video to combine.
        var process = childProcess.exec(`${__dirname}/ffmpeg.exe & ffmpeg -i ${Documents}/TYTD/MP3/${fileIdentifier}_MP3/1.m4a -b ${audioBitrate}k -ar ${audioHz} -acodec libmp3lame -preset ultrafast ${home}\\Videos\\TYTD\\${fileIdentifier}_yt-dl-cli.mp3`);
